const models =
    {
      // daily test
      'albert-base-v2':
          'bert64',  // tjs/albert-base-v2/onnx/model.onnx. TODO: NaN
      'bart-large-cnn-encoder':
          'bert64',  // tjs/facebook/bart-large-cnn/onnx/encoder_model.onnx
      ...
      'clip-vit-base-patch16':
          'clip',  // tjs/openai/clip-vit-base-patch16/onnx/model.onnx
      'densenet-9': 'img224',  // webnn
      'detr-resnet-50':
          'img224',             // tjs/facebook/detr-resnet-50/onnx/model.onnx.
                                // TODO: conformance fails
      'dino-vitb16': 'img224',  // tjs/facebook/dino-vitb16/onnx/model.onnx
      'distilbert-base-uncased':
          'bert64',  // tjs/distilbert-base-uncased/onnx/model.onnx
      'distilgpt2':
          'llm-decoder',  // tjs/gpt2/onnx/decoder_model_merged.onnx. TODO: NaN
      'efficientnet-lite4-11':
          {'images:0': ['float32', 'random', [1, 224, 224, 3]]},  // webnn
      'emotion-ferplus-8':
          {'Input3': ['float32', 'random', [1, 1, 64, 64]]},  // webnn
      'gpt2':
          'llm-decoder',  // tjs/gpt2/onnx/decoder_model_merged.onnx. TODO: NaN
      ...
      python -m samexporter.export_decoder --checkpoint
      models/sam_vit_b_01ec64.pth --output models/sam-b-decoder.onnx
      --model-type vit_b --return-single-mask python -m
      samexporter.export_encoder --checkpoint models/sam_vit_b_01ec64.pth
      --output models/sam-b-encoder.onnx --model-type vit_b --use-preprocess
      python -m samexporter.export_decoder --checkpoint
      models/sam_vit_l_0b3195.pth --output models/sam-l-decoder.onnx
      --model-type vit_l --return-single-mask python -m
      samexporter.export_encoder --checkpoint models/sam_vit_l_0b3195.pth
      --output models/sam-l-encoder.onnx --model-type vit_l --use-preprocess
      python -m samexporter.export_decoder --checkpoint
      models/sam_vit_h_4b8939.pth --output models/sam-h-decoder.onnx
      --model-type vit_h --return-single-mask python -m
      samexporter.export_encoder --checkpoint models/sam_vit_h_4b8939.pth
      --output models/sam-h-encoder.onnx --model-type vit_h --use-preprocess
      */
      'sam-b-decoder': 'sam-decoder',  // TODO: conformance fails
      'sam-b-encoder': 'sam-encoder',

      'sd-vae-decoder': 'sd-vae-decoder',
      'sd-vae-decoder-arthur':
          'sd-vae-decoder',  // https://huggingface.co/aislamov/stable-diffusion-2-1-base-onnx/tree/9f697c96d42e5c09437ff14b0a2b287366ce488d/vae_decoder
      'sd-vae-decoder-f16': 'sd-vae-decoder-f16',
      'sd-vae-encoder': 'sd-vae-encoder',


      't5-small-decoder':
          't5-decoder',  // tjs/t5-small/onnx/decoder_model_merged.onnx
      't5-small-encoder': 't5-encoder',  // tjs/t5-small/onnx/encoder_model.onnx

      'tinyyolov2-8':
          {'image': ['float32', 'random', [1, 3, 416, 416]]},  // webnn
      'whisper-tiny-decoder':
          'whisper-decoder',  // tjs/openai/whisper-tiny/onnx/decoder_model_merged.onnx
      ...
      // TODO
      'm2m100-decoder':
          'm2m100-decoder',  // https://huggingface.co/Xenova/m2m100/resolve/main/onnx/decoder_model_merged.onnx.
                             // RuntimeError: Aborted()
      'm2m100-encoder':
          'm2m100-encoder',  // https://huggingface.co/Xenova/m2m100_418M/resolve/main/onnx/encoder_model.onnx.
                             // RangeError: offset is out of bounds

      // sd-unet:
      // Stable-Diffusion-v1.5-unet-fixed-size-batch-1-float16-no-shape-ops-embedded-weights
      // from WebNN
      // sd-vae-decoder-f16: sd2.1-inpainting-vae-decoder-float16-zeroed-weights
      // from WebNN
      // the rests:
      ...
      // op unsample is deprecated
      // (https://github.com/onnx/onnx/blob/main/docs/Operators.md#Upsample), so
      // we move this to deprecated
      'candy-8': 'img224',
...
function getFeeds(session, modelName) {
    ...
    for (var k in inputNames) {
      ...
      if (v.startsWith('encoder_attention_mask')) {
        feeds['encoder_attention_mask'] =
            getTensor('int64', 1n, [1, encSeqLen]);
      }
    }
    feeds['use_cache_branch'] = getTensor('bool', false);
    feeds['input_ids'] = getTensor('int64', 99n, [1, decSeqLen]);
    feeds['encoder_hidden_states'] =
        getTensor('float32', 1, [1, encSeqLen, hiddendim]);
  }
  ...
  if (inputs == 'm2m100-decoder') {
    ...
    feeds['input_ids'] = getTensor('int64', 99n, [1, decSeqLen]);
    feeds['encoder_hidden_states'] =
        getTensor('float32', 1, [1, encSeqLen, 1024]);
  ...
  if (inputs == 'sam-decoder') {
    ...
    feeds['point_coords'] = new ort.Tensor(
        new Float32Array(
            [327.1111, 426.875, 241.77777, 341.5, 398.22223, 498.02084]),
        [1, 3, 2]);
    feeds['point_labels'] =
        new ort.Tensor(new Float32Array([0., 2., 3.]), [1, 3]);
    ...
    if (inputNames.includes('orig_im_size')) {
      feeds['orig_im_size'] =
          new ort.Tensor(new Float32Array([512., 512.]), [2]);
    }
  }
  ...
  if (['t5-decoder', 'flan-t5-decoder'].indexOf(inputs) >= 0) {
    ...
    feeds['input_ids'] = getTensor('int64', 99n, [1, decSeqLen]);
    feeds['encoder_hidden_states'] =
        getTensor('float32', 1, [1, decSeqLen, 512]);
    const encoder_shape = (inputs == 't5-decoder') ? [1, 8, encSeqLen, 64] :
                                                     [1, 6, encSeqLen, 64];
    const decoder_shape = (inputs == 't5-decoder') ? [1, 8, decSeqLen, 64] :
                                                     [1, 6, decSeqLen, 64];
    for (var k in inputNames) {
      const v = inputNames[k];
      if (v.startsWith('past_key_values.')) {
  ...
  if (inputs === 'whisper-decoder') {
    ...
    feeds['encoder_hidden_states'] =
        getTensor('float32', 'random', [1, 1500, 384]);
    for (var k in inputNames) {
      const v = inputNames[k];
      if (v.startsWith('past_key_values.')) {
        if (v.includes('decoder')) {
